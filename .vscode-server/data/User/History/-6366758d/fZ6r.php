            ...
            @foreach ($elected_courts as $e_court)
                <div class="mt-8">
                    ...
                    <select type="text" class="form-control w-16" name="attend_flg[]" required>
                        <option disabled style='display:none;'  @if ($postAttendance[$postAttendance->elected_court_id === $e_court->id]['attend_flg'] == "-") selected @endif>-</option>
                        <option value="〇" @if ($postAttendance[$postAttendance->elected_court_id === $e_court->id]['attend_flg'] == "〇") selected @endif>〇</option>
                        <option value="△" @if ($postAttendance[$postAttendance->elected_court_id === $e_court->id]['attend_flg'] == "△") selected @endif>△</option>
                        <option value="✕" @if ($postAttendance[$postAttendance->elected_court_id === $e_court->id]['attend_flg'] == "✕") selected @endif>✕</option>
                    </select>
