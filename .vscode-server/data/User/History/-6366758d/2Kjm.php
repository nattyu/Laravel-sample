            ...
            @foreach ($postAttendance as $p_attendance)
                <div class="mt-8">
                    ...
                    <select type="text" class="form-control" name="attend_flg[]" required>
                        <option disabled style='display:none;'  @if ($p_attendance->attend_flg == "-") selected @endif class="w-4">-</option>
                        <option value="〇" @if ($p_attendance->attend_flg == "〇") selected @endif class="w-4">〇</option>
                        <option value="△" @if ($p_attendance->attend_flg == "△") selected @endif class="w-4">△</option>
                        <option value="✕" @if ($p_attendance->attend_flg == "✕") selected @endif class="w-4">✕</option>
                    </select>
