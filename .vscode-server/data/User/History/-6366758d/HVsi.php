        ...
        <form method="post" action="{{ route('post-attendance.update', $postAttendance[0]) }}">
            ...
            @foreach ($elected_courts as $e_court)
                <div class="mt-8">
                    ...
                    @php
                        $filteredPostAttendances = collect($postAttendance)->filter(function ($item) use ($e_court) {
                            return $item['elected_court_id'] === $e_court->id;
                        })->toArray();
                    @endphp
                    {{ $filteredPostAttendances }}
                    <select type="text" class="form-control w-16" name="attend_flg[]" required>
                        <option disabled style='display:none;'  @if ($postAttendance[$e_court->id - 1]['attend_flg'] == "-") selected @endif>-</option>
                        <option value="〇" @if ($postAttendance[$e_court->id - 1]['attend_flg'] == "〇") selected @endif>〇</option>
                        <option value="△" @if ($postAttendance[$e_court->id - 1]['attend_flg'] == "△") selected @endif>△</option>
                        <option value="✕" @if ($postAttendance[$e_court->id - 1]['attend_flg'] == "✕") selected @endif>✕</option>
                    </select>
